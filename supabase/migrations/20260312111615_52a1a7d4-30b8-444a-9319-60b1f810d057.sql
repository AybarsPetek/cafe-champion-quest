
-- Recreate views WITHOUT security_invoker so all authenticated users can read them
DROP VIEW IF EXISTS public.profiles_public;
CREATE VIEW public.profiles_public AS
  SELECT id, full_name, avatar_url, level, total_points, created_at
  FROM profiles;

DROP VIEW IF EXISTS public.user_course_progress_public;
CREATE VIEW public.user_course_progress_public AS
  SELECT user_id, count(*) FILTER (WHERE completed = true) AS completed_courses
  FROM user_course_progress
  GROUP BY user_id;

DROP VIEW IF EXISTS public.user_badges_public;
CREATE VIEW public.user_badges_public AS
  SELECT user_id, count(*) AS badges_count
  FROM user_badges
  GROUP BY user_id;

-- Grant access to authenticated users
GRANT SELECT ON public.profiles_public TO authenticated;
GRANT SELECT ON public.user_course_progress_public TO authenticated;
GRANT SELECT ON public.user_badges_public TO authenticated;
GRANT SELECT ON public.profiles_public TO anon;
GRANT SELECT ON public.user_course_progress_public TO anon;
GRANT SELECT ON public.user_badges_public TO anon;
